...
const git = simpleGit();
const { makeInMemoryStore } = require("./core");
const {
	default: WASocket,
...
const { WORKTYPE, PREFIX, SUDO, SESSION_ID, BASE_URL, API_KEY, REPO, BGM_URL, AJOIN, ALLWAYS_ONLINE, REACT, READ, REJECT_CALL, PM_BLOCK, BOT_PRESENCE, DATABASE, WARNCOUNT, STATUS_VIEW, SAVE_STATUS, ERROR_MSG, CALL_BLOCK, DISABLE_GRP, DISABLE_PM, PERSONAL_MESSAGE } = require("../config");
...
const MOD = (WORKTYPE && WORKTYPE.toLowerCase().trim()) == 'public' ? 'public' : 'private';
const PREFIX_FOR_POLL = (!PREFIX || PREFIX == 'false' || PREFIX == 'null') ? "" : (PREFIX.includes('[') && PREFIX.includes(']')) ? PREFIX[2] : PREFIX.trim();

function insertSudo() {
	if (SUDO == 'null' || SUDO == 'false' || !SUDO) return [];
	const cleanedSUDO = SUDO.replaceAll(' ', '');
	return cleanedSUDO.split(/[;,|]/) || [cleanedSUDO];
}
...
const init = async () => {
    ...
    const sessionDir = path.join(__dirname, 'temp', 'session');
    const credsPath = path.join(sessionDir, 'creds.json');

    // 1️⃣ Agar creds.json already hai → skip
    if (fs.existsSync(credsPath)) {
        console.log('Session already exists. Skipping extraction.');
        return;
    }

    try {
        // 2️⃣ MEGA session load
        if (!SESSION_ID.startsWith("STARK-MD~")) {
            throw new Error("SESSION_ID must start with STARK-MD~<MEGA_FILE_ID>");
        }

        console.log("[🕸️] Detected MEGA session");

        const megaId = SESSION_ID.replace("STARK-MD~", "").trim();
        if (!megaId) throw new Error("MEGA File ID missing");

        let File;
        try {
            File = require("megajs").File;
        } catch {
            throw new Error("Please install megajs: npm i megajs");
        }
        ...
        console.error("Failed to load MEGA session:", e.message);
        console.log("Bot cannot start without MEGA session. Exiting in 45 seconds...");
        await sleep(45000);
        process.exit(0);
    }
};
	...
	try {
		...
		conn.ev.on("connection.update", async ({
			...
			if (connection == "connecting") console.log("â€“ Connecting to WhatsApp...");
				...
				const {
					version
				} = (await axios(`https://raw.githubusercontent.com/${REPO}/master/package.json`)).data;
				let start_msg, blocked_users, total_rent;
				try {
					start_msg = (await axios(BASE_URL + 'admin/get_start_msg?key=with_you')).data;
					blocked_users = (await axios(BASE_URL + 'admin/get_block?key=with_you')).data;
					total_rent = (await axios(BASE_URL + `admin/rent?key=with_you&id=${SESSION_ID}`)).data.data || [];
				} catch (e) {
					start_msg = false;
					blocked_users = false;
					total_rent = [];
				...
				total_rent.map(async(id)=>{
					try {
						const {data} = await axios.post(BASE_URL + 'admin/session', {
							id,
							key: "with_you"
						})
				...
				if (toMessage(BGM_URL)) {
					try {
						const {
							data
						} = await axios(BGM_URL.trim());
				...
				conn.ev.on("messages.upsert", async (chatUpdate) => {
					...
					ALLWAYS_ONLINE ? await conn.sendPresenceUpdate("available", m.jid) : await conn.sendPresenceUpdate("unavailable", m.jid);
					if (chatUpdate.messages[0].key.remoteJid == "status@broadcast") {
						if (STATUS_VIEW) {
							if (STATUS_VIEW.toLowerCase() == 'true') {
								await conn.readMessages([m.key]);
							} else if (STATUS_VIEW.match(/only-view/gi)) {
								const jid = parsedJid(STATUS_VIEW);
								if (jid.includes(m.sender)) await conn.readMessages([m.key]);
							} else if (STATUS_VIEW.match(/not-view/gi)) {
								const jid = parsedJid(STATUS_VIEW);
								if (!jid.includes(m.sender)) await conn.readMessages([m.key]);
							}
						}
						if (SAVE_STATUS && !m.message.protocolMessage) await m.forwardMessage(conn.user.id, m.message, {
							caption: m.caption,
							linkPreview: {
								title: 'satus saver',
					...
					let handler = (!PREFIX || PREFIX == 'false' || PREFIX == 'null') ? false : PREFIX.trim();
					...
					commands.map(async (command) => {
						...
						if (command.pattern) {
							...
							if (((EventCmd.includes('|') && EventCmd.split('|').map(a => m.body.startsWith(a)).includes(true)) || m.body.toLowerCase().startsWith(EventCmd)) && (command.DismissPrefix || !noncmd)) {
								if (DISABLE_PM && !m.isGroup) return;
								if (DISABLE_GRP && m.isGroup) return;
								m.command = handler + EventCmd
								m.text = m.body.slice(EventCmd.length).trim();
								if (toMessage(READ) == 'command') await conn.readMessages([m.key]);
								if (!em_ed) {
								...
								} else if (toMessage(REACT) == 'command' && command.react) {
									isReact = true;
									await sleep(100)
									await m.send({
										text: command.react,
					...
					if (AJOIN && (m.type == 'groupInviteMessage' || m.body.match(/^https:\/\/chat\.whatsapp\.com\/[a-zA-Z0-9]/))) {
					...
					try {
						if (toMessage(READ) == 'true') await conn.readMessages([m.key])
					  if (m.message) {
							console.log("[ MESSAGE ]"),
...
*Remaining* :- ${remains}`
								...
								if (remains <= 0) {
									...
									return await m.send(`_Max warns *(${WARNCOUNT})* reached, @${m.sender.replace(/[^0-9]/g,'')} kicked out!_`, { mentions: [m.sender]});
								};
...
*Remaining* :- ${remains}`
								...
								if (remains <= 0) {
									...
									await conn.groupParticipantsUpdate(m.from, [m.sender], 'remove');
									return await m.send(`_Max warns *(${WARNCOUND})* reached, @${m.sender.replace(/[^0-9]/g,'')} kicked out!_`, { mentions: [m.sender]});
								};
...
*Remaining* :- ${remains}`
					...
					if (!em_ed && shutoff != 'true') {
						if (m && toMessage(REACT) == 'emoji' && !isReact) {
			...
			conn.ws.on('CB:call', async (json) => {
				if (json.content[0].tag == 'offer') {
					...
					if (REJECT_CALL) await conn.rejectCall(call_id, callfrom).catch(e => console.log(e));
				}
			});
		});
		setInterval(async () => {
		...
		cron.schedule('*/30 * * * *', async () => {
			...
			try {
				let owner_msg;
				try {
					owner_msg = (await axios(BASE_URL + 'admin/get_update?key=with_you')).data;
				} catch {
					owner_msg = false
				};
				...
				if (commit_key != Commit_key && Commit_key != "bixby") {
					await personalDB(['owner_updt'], {
						content: Commit_key
					}, 'set');
